...
function addHTMLCruiseTable(cru) {
	let t = '<td>'+cru.name_auto+'</td>';
		...
		t+= '<td>'+cru.comment+'</td>';
		t+= '<td>'+cru.created_at+'</td>';
		t+= '<td>'+cru.updated_at+'</td>';
...
function loadingSession(id){
	...
	$.post('/log/constructor/'+id, {'_token': csrftoken, 'id': id}, function(r){ return createSession(r); });
}
...
function dragMarker(id) {
	...
	$('#markerCords').empty().append(' (гео '+position_x+' : '+position_y+')');
}
